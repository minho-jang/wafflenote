...
from krwordrank.sentence import keysentence, make_vocab_score
...
def get_stopwords():
    with open('text_analysis_beta/stopword_list.txt', 'r') as f:
        default_stopwords = [x for x in f.read().split('\n') if len(x) > 0]
...
def get_key_sentences(text, num):
    ...
    if len(sentences) < num:
        return ["KeySentencesError: Number of sentences must be bigger than num"]
